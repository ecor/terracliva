...
#######################################
# R Script by Emanuele Cordano       #
# Date: 2024-03-20                   #
# Description:                      #
# - Dry Spell Climate Variability Analysis vs crop yield date 
# -   Area: Lake Tanganyika (Ruzizi River) Basin 
# -   Source Data: CHIRPS,https://www.nature.com/articles/s41597-020-0433-7, https://doi.pangaea.de/10.1594/PANGAEA.909132
#######################################
...
precipitation_dataset_path <- "/home/ecor/local/data/climate/precipitation/chirps"
precipitation_dataset_daily <- "%s/daily/ruzizi_river_basin/chirps_daily_ruzizi_river_basin_%04d.tif" %>% sprintf(precipitation_dataset_path,years) %>% rast()
###"/home/ecor/local/data/climate/precipitation/chirps/daily/ruzizi_river_basin/chirps_daily_ruzizi_river_basin_1982.tif
############### DRYSPELL
valmins=c(1,3,5,8)
names(valmins) <- sprintf("valmin_%03d",valmins)
months <- c(12,1,2)

drys_grd_format <- "/home/ecor/local/rpackages/jrc/terracliva//inst/ext_data/results/dryspell/drysp_ex1_ruzizi_valmin_%03d.grd"
drys_grd_aggr_format <- "/home/ecor/local/rpackages/jrc/terracliva//inst/ext_data/results/dryspell/drysp_ex1_ruzizi_aggr_valmin_%03d.grd"
...
for (nv in names(valmins)) {
  ...
  if (cond) {
    ...
    drys[[nv]] <- dryspellapprast(precipitation_dataset_daily,valmin=valmin,months=months,cores=cores,filename=filename,overwrite=TRUE)
  }
...
##  cond <- FALSE
  if (cond) {
    ...
    drys_aggr[[nv]] <- aggregate(drys[[nv]],filename=filename_aggr,fact=10,fun="max",cores=cores,overwrite=TRUE)
...
### HYSTOGRAM 
ga <- ggplot()+geom_bar(aes(drys_class),data=out7)+theme_bw()
